...
public strictfp class LandscaperRobot extends Robot {
   ...
   private int netgunDistance = 10000; //big number
   ...
   public void run() throws GameActionException {
      //boolean droneDetected = false;   //no drones seen yet, reset
      int droneDist = 100;
   ...
   private void moveTurtle(MapLocation ml) throws GameActionException {
      ...
      for (int i = 8; i-->0; ) {
         d = dirs[i];
         m = location.add(d);
         if (Utility.chebyshev(m, hqLocation) != 1) 
            continue;
         if (rc.canSenseLocation(m)) {
            elev = rc.senseElevation(m);
            elevDiff = robotElevation - elev;
            if (elevDiff < -MAX_HEIGHT_THRESHOLD) 
               continue;
   ...
   private void moveTerraform(MapLocation ml) throws GameActionException {
      ...
      for (int i = 8; i-->0; ) {
         d = dirs[i];
         m = location.add(d);
         if (pitTile(m) || Utility.chebyshev(m, hqLocation) == 1) 
            continue;
         if (rc.canSenseLocation(m)) {
            botInfo = rc.senseRobotAtLocation(m);
            if (botInfo != null && botInfo.team == team && botInfo.type.isBuilding()) 
               continue;
            elev = rc.senseElevation(m);
            elevDiff = robotElevation - elev;
            if (elevDiff < -MAX_HEIGHT_THRESHOLD) 
      ...
      if (nearestFillTile == null) {
         ...
         for (int x = Math.max(0, location.x - radius); x <= Math.min(mapWidth - 1, location.x + radius); x++) {
            for (int y = Math.max(0, location.y - radius); y <= Math.min(mapHeight - 1, location.y + radius); y++) {
               ...
               if (csDist > MAX_TERRAFORM_RANGE) 
                  continue;
               if (pitTile(ml)) 
                  continue;
               elev = rc.senseElevation(ml);
               ri = rc.senseRobotAtLocation(ml);
               if (ri != null && (ri.type.isBuilding() || (ri.team == team && ri.type == RobotType.LANDSCAPER))) 
                  continue;
