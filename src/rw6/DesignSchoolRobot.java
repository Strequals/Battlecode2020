...
public strictfp class DesignSchoolRobot extends Robot {
	...
	public void run() throws GameActionException {
		...
		if (round < 500 || soup > RobotType.VAPORATOR.cost + RobotType.LANDSCAPER.cost) {
		...
		for (int i = 4; i-->0;) {
			...
			int lr = Utility.chebyshev(location.add(left), hqLocation);
			int rr = Utility.chebyshev(location.add(right), hqLocation);
			if (lr == 2 && rc.canBuildRobot(RobotType.LANDSCAPER, left)) {
				rc.buildRobot(RobotType.LANDSCAPER, left);
				return;
			}
			if (rr == 2 && rc.canBuildRobot(RobotType.LANDSCAPER, right)) {
